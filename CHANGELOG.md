...
**The variation gallery graduated to 100%, and our canary hunk went dead (`src/Internal/VariationGallery/Package.php`). Tier 1.** 11.1.0 turns `Package::is_enabled()` into a hardcoded `return true`, deletes the `is_enabled()` guard from `init()`, deprecates `is_in_canary_cohort()` into a proxy that just calls `is_enabled()`, and sets the feature definition to `enabled_by_default => true`, `is_experimental => false`, `disable_ui => true`, `deprecated_since => '11.1.0'`, `deprecated_value => true`. Between them there is **no option, filter or Settings toggle left that can hold it back**: `FeaturesController::feature_is_enabled()` returns `deprecated_value` before it ever reads the option, and nothing calls `is_enabled()` any more. The drift watchlist is what caught it — the `is_in_canary_cohort` count dropped from 2 to 1, which is exactly the "hunk still fits but no longer does anything" failure the watchlist exists to detect.

Held back by explicit decision, preserving merchant opt-in, with two hunks. `is_enabled()` is rewritten to `return 'yes' === get_option( self::ENABLE_OPTION_NAME, '' )`, restoring the 10.9.x/11.0.x contract where an explicit `yes` opts in and anything else stays off. The `if ( ! self::is_enabled() ) { return; }` guard is restored at the top of `init()` — **without that second hunk the first one achieves nothing operationally**, because the admin hooks still register and `maybe_schedule_migration()` still queues the Additional Variation Images data migration regardless of whether the feature is on. The old `is_in_canary_cohort()` early-return hunk is retired: the method is now a deprecated proxy that nothing consults.

This is a one-release reprieve, not a permanent fork. **11.1.0 is the last release that holds the variation gallery back** — the hunks retire at 11.2, the gallery UI switches on across the fleet at that upgrade, and the AVI migration queues then. The feature is local first-party functionality with no phone-home; upstream has shipped it at 100%; permanently diverging from core product UI is not what this patch set is for. Holding it for one cycle gives clients warning.

**New target — `LegacySelect2UsageTracker` (`includes/class-woocommerce.php`). Tier 1.** New in 11.1.0 at `src/Internal/LegacyAssets/LegacySelect2UsageTracker.php`, registered from `WooCommerce::init()`. It fingerprints which plugins on the store still enqueue the legacy `select2` / `wc-select2` script handles — the handles, their dependents, and the **plugin-relative path of every dependent script** — and ships that as a `legacy_select2_usage_detected` event, transient-throttled to weekly per distinct payload. That is an inventory of the store's installed plugins, which is precisely the kind of fingerprinting this patch set exists to stop. The admin path is gated on `WC_Site_Tracking::is_tracking_enabled()`, which we already sever, but **the frontend path hooks `wp_print_footer_scripts` unconditionally** and routes through `WC_Analytics_Tracking::add_event_to_queue()` — the separate `woocommerce-analytics` plugin — so it sits outside our master switch. Commenting out the `->register()` call kills both paths at once, in a file we already patch, for one line. This is the `+1` behind `WC_Tracks::record_event` going from 58 to 59 between 11.0.1 and 11.1.0.

**New target — pin the shipped `off` default of every feature that currently ships off (`includes/class-woocommerce.php`). Tier 1.** Raised across the 11.0.0 and 11.0.1 cycles and built here. The block sits in `WooCommerce::init()` immediately after the `woocommerce_remote_variant_assignment` force — placement matters, because `init()` opens by calling `register_additional_features()`, so the feature definitions are fully populated by the time it runs. It iterates `FeaturesController::get_features( true, false )` and, for each feature that is not already `enabled_by_default` and is not `deprecated_since`, reads the option with a `'__wpatcher_absent__'` sentinel default and writes `'no'` **only when the option is genuinely absent**.

This is write-once materialisation of the current default, **not a force**: a merchant who later enables something keeps it enabled. The point is narrow — features graduate over time, and a release that flips an `enabled_by_default` from `false` to `true` would otherwise switch that feature on for every store that had never touched the toggle. Materialising the default now means the option exists, so the new default is never consulted. 23 options are pinned as of 11.1.0. Option names come from the public `FeaturesController::feature_enable_option_name()` rather than being built by hand, because several features carry an `option_key` override (`analytics`, both HPOS options, `wc-visual-attribute`, `cart_save_for_later`, `product_wishlist`, `ProductMediaGallery`, `BlockEditorUnifiedAssets`) and guessing `woocommerce_feature_<slug>_enabled` would write orphan options that nothing reads. The sentinel matters for the same reason a falsy test would not do: `absent` has to be distinguishable from an explicit `no`, or the option gets rewritten on every load. There is upstream precedent for the shape — `wc_update_1050_enable_autoload_options()` materialises four feature options with the same absent-check-then-write pattern.

**The caveat, learned the same day it was built:** this defends against an `enabled_by_default` flip and nothing else. It **cannot** hold back a feature that upstream hardcodes on and marks deprecated, because `feature_is_enabled()` short-circuits on `deprecated_value` before it reads the option at all. That is exactly what 11.1.0 did to the variation gallery, and why that one needed its own hunk. When a feature you care about comes on anyway, check *which* mechanism turned it on before assuming the pin failed.

**One hunk re-rolled for cosmetic drift (`src/Internal/Admin/Settings/Payments.php`).** Upstream hoisted `if ( current_user_can( 'install_plugins' ) )` into a `$can_install_plugins` variable a few lines above our target, which shifted the context enough to reject. Our target line is untouched; the hunk was re-rolled by hand against the new context. `Payments.php` line 237 (`get_payment_extension_suggestions()`) remains deliberately unpatched — its callee `PaymentsProviders::get_extension_suggestions()` is already early-returned.

**Analysis — no other new phone-home targets.** Full-tree pattern counts (occurrences across all `*.php`) are flat between 11.0.1 and 11.1.0 apart from the Select2 tracker: `wp_remote_*` 185, `pixel.wp.com` 4, `tracking.woocommerce.com` 1, `public-api.wordpress.com` 12, `DataSourcePoller` 122, `stats.wp.com` 2, and `WC_Tracks::record_event` 58 → 59. That single increment is the Select2 tracker: comparing the files carrying `record_event` between the two releases, the only addition is `LegacySelect2UsageTracker.php`. The one other movement is a refactor, not new telemetry — the `local_pickup_save_changes` event moved out of `Blocks/Shipping/ShippingController.php` into a new `PickupLocationsRestController.php`, unchanged and still severed by the master switch. Counted again between clean rc.1 and clean final, every one of them is unchanged. Three net-new features all ship `enabled_by_default => false` and are therefore inert: `order_withdrawal` (a public `/my-account/withdraw-order/` page for EU withdrawal requests — not experimental, so it is UI-visible in Settings → Advanced → Features), `product_media_gallery` (gallery videos) and `BlockEditorUnifiedAssets`. `src/Internal/VariationGallery/Telemetry.php` is **not** new — it was present in 11.0.1 — and is inert regardless, since it only filters `woocommerce_tracker_data` and the tracker send is already severed.

The rc.1 → final changes are all local hardening with no outbound calls: GraphQL query complexity and depth rules, a new `OrderLineMetaValidator`, REST authentication and order-controller fixes, and a type-safety fix in `WooPaymentsService::get_nox_profile()`. `src/Admin/Features/Features.php` gained a private `is_analytics_enabled()` that reads the analytics option directly instead of going through `FeaturesController`; it does not interact with the feature pin, because `analytics` is `enabled_by_default => true` and the pin skips it.

**Drift watchlist — all anchors at baseline against clean 11.1.0**, despite the admin bundle having been rebuilt between 11.0.1 and this release: marketplace banner 3 (plus 9 CSS), footer 4 (plus 7 CSS), `wc_featuredBannerDismissed` 2, `isJetpackConnected` 3, `woocommerce_remote_variant_assignment` 1 app + 1 embed, `order_attribution_install_banner_dismissed` 2 + 2, `createStatsdURL` 2, `case 'range'` still bounded at both ends, `record_gateway_event()` present and unchanged. The only movement is `is_in_canary_cohort` 2 → 1, explained above.

**Two forced options are now dead weight, and neither has a working replacement.** Recorded here rather than silently carried:

- `woocommerce_feature_point_of_sale_enabled` — `point_of_sale` has been `deprecated_since => '11.0.0'` with `deprecated_value => true` since 11.0.0, so `feature_is_enabled()` returns `true` without ever reading the option. Our forced `'no'` has done nothing since then. There is no option, filter or hook that can turn it off. In practice this costs nothing: no PHP in the tree gates behaviour on the flag — the only references outside `FeaturesController` are the one-time update function that writes it and its registration — and the feature description is explicit that it enables POS "in the WooCommerce mobile apps".
- `woocommerce_feature_reactify-classic-payments-settings_enabled` — the slug no longer exists in `FeaturesController::get_feature_definitions()` at all. The only surviving reference anywhere in the tree is `wc_update_985_enable_new_payments_settings_page_feature()`, which writes it to `'yes'`; nothing reads it. The reactified payments settings UI is no longer gated on it.

Both are left in the `$options_to_no` list for now. The loop is guarded — it writes only when the current value is truthy — so a dead entry costs one autoloaded `get_option()` per request and no DB write. Retiring them is a separate decision, not something to fold into a release patch.

**Verification.** Candidate applied to a pristine final 11.1.0 extraction with zero rejects, zero fuzz and zero offset; `php -l` clean across all 22 patched PHP files and `node --check` clean on the one patched JS file; round-trip verified, meaning the patch re-applied to a clean tree reproduces the built tree byte for byte. Both `WCAdminAssets.php` inline scripts still carry the load-bearing leading `;`. The `PATCHED` badge is bumped to `2026-09-03`.

---

## 11.0.1 — 2026-08-11

**Revised 2026-08-11** to add one new target (below). The WooCommerce-source analysis in the following paragraphs is unchanged, as is the "bump-only" description of how the patch was derived from 11.0.0. The file count goes from 22 to 23.

**New target — the order attribution install banner, and a self-inflicted cohort bug (`src/Admin/API/Options.php`).** Analytics → Overview was showing a card headed "Discover what drives your sales" with **Try it now** and **Dismiss** buttons (`.woocommerce-order-attribution-install-banner`). It is an upsell for the `woocommerce-analytics` plugin: clicking through installs and activates it, and the banner fires `order_attribution_install_banner_viewed`, `_clicked` and `_dismissed` Tracks events.

**The 11.0.0 patch is what put the store in the bucket that shows it.** The banner is gated client-side on `woocommerce_remote_variant_assignment`, the same 1–120 rollout bucket that patch forces to `0`. Every server-side `range` rule is bounded at both ends (`>= min && <= max`), so `0` correctly matches nothing. The wc-admin client is not so careful: the banner's check is `parseInt( value ) <= threshold`, upper bound only, and the threshold defaults to 12. So `0` passes, and the banner appears on a store that would have seen it only 10% of the time unpatched. Forcing the bucket to `0` is still right for the PHP side, and the two other client-side surfaces we suppress are unaffected, but the general hazard is worth recording: **a sentinel chosen to sit below every server-side range will pass any client-side check that only tests an upper bound.**

Fixed by blanking the option in the wc-admin options REST response, in `Options::get_options()`. `false` reaches the client as `NaN` through `parseInt()`, which fails an upper-bound test and a lower-bound one alike, so no present or future client-side cohort check can match. That is strictly better than swapping `0` for a large sentinel, which would only move the problem to the other bound. This is the only client-side consumer of the option in either admin bundle (`app` and `embed`), nothing in the browser has a legitimate use for a server-side rollout bucket, and the stored value is untouched, so `Package::is_in_canary_cohort()` and the Remote Spec Engine rules still see `0`.

Blanking it inside the loop rather than dropping it from the `$legacy_whitelisted_options` allowlist above is deliberate. `get_item_permissions_check()` rejects the **whole** request if any single requested option is not permitted, so removing it there would 403 every batch that asks for it alongside options the client does need. Note also that `woocommerce_remote_variant_assignment` is not in any `woocommerce_admin_preload_options` list, so the REST controller is the only path by which the client can obtain it. Patching the compiled bundle was rejected for the usual reason: `assets/client/admin/app/index.js` is minified onto a single 248 KB line, so the hunk would embed that line twice.

Verified with a dry-run against a pristine 11.0.1 extraction: zero rejects, zero fuzz, zero offset, and `php -l` clean. Then deployed via `wpatch` and **confirmed on `devx.headwall.tech`**: the banner is gone from Analytics → Overview. That validates the full 23-file patch on a live site, not just this hunk.
